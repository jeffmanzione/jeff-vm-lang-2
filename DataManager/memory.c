...
AllocInfo alloc_info(uint32_t elt_size, uint32_t count, uint32_t line,
    const char type_name[], const char func[], const char file[]) {
  AllocInfo info =
      { .elt_size = elt_size, .count = count, .line = line, .type_name = strdup(
          type_name), .func = strdup(func), .file = strdup(file) };
  return info;
}
...
void alloc_finalize() {
  ...
  void embarrass_me(void *ptr) {
    ...
    fprintf(stderr, "Forgot to free %p(%sx%d) allocated at %s:%d in %s(...)\n",
        ptr, info->type_name, info->count, info->file, info->line, info->func);
    fflush(stderr);
    DEALLOC(ptr);
  }
...
void *realloc__(void *ptr, uint32_t elt_size, uint32_t count, uint32_t line,
  ...
  *((AllocInfo *) new_info_ptr) = alloc_info(elt_size, count, line,
      old_info.type_name, func, file);
...
//  //DEBUGF("'%s'[%d:%d]='%s'", src, start_index, end_index, target);
  return target;
}

//char *string_copy_after(const char src[], int start_index) {
//  return string_copy_range(src, start_index, strlen(src));
//}
//
//char *string_copy(const char src[]) {
//  return string_copy_after(src, 0);
//}

//void string_prepend(char **src, const char head[]) {
//  ASSERT(NOT_NULL(src), NOT_NULL(head));
//  int existing_len = strlen(*src);
//  *src = REALLOC(*src, char, strlen(*src) + strlen(head) + 1);
//  memmove(*src + existing_len, *src, strlen(*src) + 1);
//  memcpy(*src, head, strlen(head));
//}

//void string_append(char **src, const char tail[]) {
//  ASSERT(NOT_NULL(src), NOT_NULL(tail));
//  int existing_len = strlen(*src);
//  *src = REALLOC(*src, char, strlen(*src) + strlen(tail) + 1);
//  strcpy(*src + existing_len, tail);
//}
...
//    const char *part = va_arg(valist, const char *);
//    ASSERT_NOT_NULL(part);
//    if (NULL == result) {
//      result = string_copy(part);
//    } else {
//      string_append(&result, part);
//    }
//  }
//  va_end(valist);
//  return result;
