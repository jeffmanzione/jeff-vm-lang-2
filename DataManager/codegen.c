  ...
  if (has_else) {
    ...
    num_lines += lines_for_else
        + tape_ins_int(tape, IF,
            lines_for_else + 1 /* for the jmp to end */,
...
int codegen_tuple(ExpressionTree *tree, int *argument_count, Tape *tape) {
  ...
  if (tree->second->second == NULL
    ...
    num_lines += codegen_tuple(tree->second->second, argument_count, tape);
    (*argument_count)++;
...
int codegen_class(ExpressionTree *tree, Tape *tape) {
  ...
  int num_lines = 0;
  num_lines += tape_class(tape, tree->second->first->token);
  if (is_leaf(tree->second->second->first)
      && tree->second->second->first->token->type == LBRCE) {
    num_lines += codegen(tree->second->second->second->first, tape);
  } else {
    num_lines += codegen(tree->second->second, tape);
  }
  num_lines += tape_endclass(tape, tree->second->first->token);
  return num_lines;
}
...
int codegen_anon_function(ExpressionTree *tree, Tape *tape) {
  ...
  {
    bool no_args = arg_begin->first->token != NULL
        && arg_begin->first->token->type == RPAREN;
    if (no_args) {
      lines_for_func += codegen(arg_begin->second, tmp_tape);
    } else if (arg_begin->first->expression != function_argument_list) {
      lines_for_func += tape_ins(tmp_tape, SET, arg_begin->first->token);
      lines_for_func += codegen(arg_begin->second->second, tmp_tape);
    } else {
      lines_for_func += codegen(arg_begin->first, tmp_tape);
      lines_for_func += codegen(arg_begin->second->second, tmp_tape);
    }
...
int codegen_function(ExpressionTree *tree, Tape *tape) {
  ...
  int num_lines = tape_label(tmp_tape, tree->second->first->token);
  int lines_for_func = 0;
  {
    bool no_args = arg_begin->first->token != NULL
        && arg_begin->first->token->type == RPAREN;
    ...
    if (0 == strcmp(NEW_KEY, tree->second->first->token->text)) {
      lines_for_func += tape_ins_text(tmp_tape, RES, THIS,
          tree->second->first->token);
    }
    lines_for_func += tape_ins_no_arg(tmp_tape, RET,
        tree->second->first->token);
  }
  num_lines += tape_ins_int(tape, JMP, lines_for_func,
      tree->second->first->token);
  tape_append(tape, tmp_tape);
  tape_delete(tmp_tape);
...
//DEBUGF("codegen_");
  ...
  if (prod == identifier || prod == string_literal || prod == constant) {
    ...
    if (!is_leaf(tree->second)) {
      int num_args = 1;
      num_lines += codegen_tuple(tree->second->first->second, &num_args, tape)
          + codegen(tree->second->first->first, tape)
          + tape_ins_no_arg(tape, PUSH, tree->first->token)
    ...
    int num_args = 1;
    num_lines += codegen_tuple(tree->second, &num_args, tape)
