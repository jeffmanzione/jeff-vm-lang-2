...
Element Int__(VM *vm, Thread *t, ExternalData *data, Element *arg) {
  ...
  if (NONE == arg->type) {
    ...
    int_val = obj_get_field_ptr(arg, ADDRESS_KEY).val.int_val;
...
Element Float__(VM *vm, Thread *t, ExternalData *data, Element *arg) {
  ...
  if (NONE == arg->type) {
    ...
    double_val = (double)obj_get_field_ptr(arg, ADDRESS_KEY).val.int_val;
  } else if (VALUE == arg->type) {
...
Element Char__(VM *vm, Thread *t, ExternalData *data, Element *arg) {
  ...
  if (NONE == arg->type) {
    ...
    char_val = (int8_t)obj_get_field_ptr(arg, ADDRESS_KEY).val.int_val;
  } else if (VALUE == arg->type) {
