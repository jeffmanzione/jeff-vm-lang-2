...
Function populate_function_variant(const SyntaxTree *stree, ParseExpression def,
  ...
  Function func = { .def_token = NULL, .fn_name = NULL, .const_token = NULL,
      .has_args = false, .is_const = false, .body = NULL };
    ...
    if (arg->has_default) {
      ...
      num_ins += tape->ins_int(tape, IFN, 3, arg->arg_name)
          + tape->ins_no_arg(tape, (i == num_args - 1) ? RES : PEEK,
              arg->arg_name) + tape->ins_int(tape, TGET, i, arg->arg_name)
          + tape->ins_int(tape, JMP, default_ins, arg->arg_name) + default_ins;
...
int produce_arguments(Arguments *args, Tape *tape) {
  ...
  defaults_ins += defaults->ins_int(defaults, JMP, nondefaults_ins,
      first->arg_name);
...
ModuleDef populate_module_def(const SyntaxTree *stree) {
  ...
  module_def.statements = expando(ExpressionTree*, DEFAULT_EXPANDO_SIZE);
...
void delete_module_def(ModuleDef *module) {
  expando_delete(module->imports);

  expando_iterate(module->classes, (void (*)(void*)) delete_class);
  expando_delete(module->classes);

  expando_iterate(module->functions, (void (*)(void*)) delete_function);
  expando_delete(module->functions);

  void delete_statement(void *ptr) {
    ExpressionTree *statement = *((ExpressionTree**) ptr);
    delete_expression(statement);
  }
  expando_iterate(module->statements, delete_statement);
  expando_delete(module->statements);
}
...
int produce_module_def(ModuleDef *module, Tape *tape) {
  ...
  if (module->name.is_named) {
    num_ins += tape->module(tape, module->name.module_name);
  }
  void produce_imports(Import *import) {
    num_ins += tape->ins(tape, LMDL, import->module_name);
  }
  expando_iterate(module->imports, (void (*)(void*)) produce_imports);

  void produce_statement(void *ptr) {
    ExpressionTree *statement = *((ExpressionTree**) ptr);
    num_ins += produce_instructions(statement, tape);
  }
  expando_iterate(module->statements, produce_statement);
  num_ins += tape->ins_int(tape, EXIT, 0, NULL);

  void produce_class_helper(Class *class) {
    num_ins += produce_class(class, tape);
  }
  expando_iterate(module->classes, (void (*)(void*)) produce_class_helper);

  void produce_function_helper(Function *func) {
    num_ins += produce_function(func, tape);
  }
  expando_iterate(module->functions, (void (*)(void*)) produce_function_helper);
  return num_ins;
}
