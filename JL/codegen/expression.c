...
#define Populate(name, stree) if (tree->expression == name) \
                                return Populate_##name(stree)
#define Codegen(name, etree, tape)  if (tree->type == name) \
                                      return Produce_##name(&etree->name, tape)
...
ImplProduce(identifier, Tape *tape) {
  return tape_ins(tape, RES, identifier->id);
}
