...
void tape_append(Tape *head, Tape *tail) {
  ...
  void insert_class_parent(Pair * kv) {
    Expando *parents = expando(char *, 2);
    void insert_parent_name(void *ptr) { expando_append(parents, &ptr); }
...
void tape_write_range(const Tape *const tape, int start, int end, FILE *file) {
  ...
  for (i = start; i < end; i++) {
    ...
    if (NULL != (text = map_lookup(&i_to_class_starts, (void *)i))) {
      ...
      if (NULL != parents) {
        void add_parent(void *ptr) { fprintf(file, ",%s", *((char **)ptr)); }
...
void tape_read_binary(Tape *const tape, FILE *file) {
  ...
  for (i = 0; i < num_classes; i++) {
    ...
    char *class_name =
        *((char **)expando_get(strings, (uint32_t)class_name_index));
    ...
    if (num_parents > 0) {
      ...
      for (j = 0; j < num_parents; j++) {
        ...
        char *parent_name =
            *((char **)expando_get(strings, (uint32_t)parent_name_index));
    ...
    for (j = 0; j < num_methods; j++) {
      ...
      char *method_name =
          *((char **)expando_get(strings, (uint32_t)method_name_index));
...
void tape_write_binary(const Tape *const tape, FILE *file) {
  ...
  void map_insert_string(Pair * kv) { insert_string(kv->key); }
  void set_insert_string(void *ptr) { insert_string(ptr); }
  void expando_insert_string(void *ptr) { insert_string(*((char **)ptr)); }
  ...
  void add_ref(Pair * kv) {
    uint16_t ref_name_index = (uint16_t)(int)map_lookup(&string_index, kv->key);
  ...
  void add_class(Pair * kv) {
    uint16_t class_name_index =
        (uint16_t)(int)map_lookup(&string_index, kv->key);
    uint16_t class_start =
        (uint16_t)(int)map_lookup(&tape->class_starts, kv->key);
    uint16_t class_end = (uint16_t)(int)map_lookup(&tape->class_ends, kv->key);
    ...
    if (NULL == parents) {
      ...
      void add_parent(void *ptr) {
        uint16_t parent_name_index =
            (uint16_t)(int)map_lookup(&string_index, *((char **)ptr));
    ...
    void add_methods(Pair * kv2) {
      uint16_t method_name_index =
          (uint16_t)(int)map_lookup(&string_index, kv2->key);
